 ...
 *   2. Create project "Discover Hawaii"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Hawaii Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'hawaii'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Hawaii ${year}`,
      description: `${title}\n\nFull guide: https://discoverhawaii.info/${slug}/`,
      tags: [`Hawaii ${slug}`, 'Hawaii travel', `Hawaii ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Hawaii 🌺 #shorts`,
      description: `${title} — Full guide: https://discoverhawaii.info/destinations/${slug}/`,
      tags: [`${title} Hawaii`, 'Hawaii travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Hawaii — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Hawaii for over 15 years with deep local knowledge. No sponsored content.
...
function generateDestTags(slug, title, dest) {
  ...
  if (dest?.region) tags.push(`${dest.region} Hawaii`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER HAWAII             ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
