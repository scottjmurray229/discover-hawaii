...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'beach': ['waikiki', 'kailua', 'poipu', 'wailea'],
    'surf': ['north-shore', 'waikiki', 'kona'],
    'surfing': ['north-shore', 'waikiki', 'kona'],
    'volcano': ['volcanoes-national-park', 'haleakala', 'mauna-kea'],
    'snorkel': ['kailua', 'poipu', 'kona'],
    'hike': ['diamond-head', 'waimea-canyon', 'na-pali-coast', 'haleakala'],
    'hiking': ['diamond-head', 'waimea-canyon', 'na-pali-coast', 'haleakala'],
    'honeymoon': ['wailea', 'poipu', 'na-pali-coast'],
    'romantic': ['wailea', 'poipu', 'lahaina'],
    'family': ['waikiki', 'kailua', 'poipu', 'kona'],
    'luxury': ['wailea', 'poipu', 'kona'],
    'budget': ['waikiki', 'north-shore', 'hilo'],
    'food': ['honolulu', 'lahaina', 'kona'],
    'culture': ['honolulu', 'molokai', 'pearl-harbor'],
    'history': ['pearl-harbor', 'honolulu', 'lahaina'],
    'nature': ['na-pali-coast', 'waimea-canyon', 'volcanoes-national-park'],
    'adventure': ['na-pali-coast', 'mauna-kea', 'road-to-hana'],
    'relaxation': ['wailea', 'poipu', 'kailua'],
    'stargazing': ['mauna-kea', 'haleakala'],
    'coffee': ['kona', 'hilo'],
  };
  ...
  return ['waikiki', 'north-shore', 'haleakala'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Hawaii AI Trip Planner. You generate detailed day-by-day itineraries for Hawaii travel.
...
- Be specific: real place names, real prices, real transport options (inter-island flights, rental cars, etc.)
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-waikiki"
- Recommend inter-island flights or ferries when trips span multiple islands
...
{
  "title": "string — trip title like '7-Day Oahu & Maui Discovery'",
      ...
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Diamond Head', 'Waikiki Beach', 'Haleakala Crater'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Hawaii trip:\n${parts.join('\n')}`;
